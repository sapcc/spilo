...
bootstrap:
  ...
  clone_with_wale:
    command: python3 /scripts/clone_with_wale.py --envdir "{{CLONE_WALE_ENV_DIR}}" --recovery-target-time="{{CLONE_TARGET_TIME}}"
    recovery_conf:
...
def get_placeholders(provider):
    ...
    placeholders['WALE_BINARY'] = 'wal-g' if use_walg else 'wal-e'
...
def write_wale_environment(placeholders, provider, prefix, overwrite):
    ...
    if wale.get('WAL_S3_BUCKET'):
        ...
        if not aws_region:
            ...
            # match = re.search(r'.*(eu-\w+-\d+).*', wale['WAL_S3_BUCKET']) db-backup-pgcluster
